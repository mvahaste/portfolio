...
// Add 'prose' boolean prop to make it use the prose class
export function TypographyP({ children, prose = false }: { children: React.ReactNode; prose?: boolean }) {
	return <p className={`leading-7 [&:not(:first-child)]:mt-6 ${prose ? "max-w-prose" : ""}`}>{children}</p>;
}
